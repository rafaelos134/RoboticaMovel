...
def update_map_log_odds(log_odds_map, robot_pos, laser_data, step, last_seen,
    ...
    for d, ang in laser_data: # verificar como esse laiser data está chegando
        ...
        # 🔧 MODIFICAÇÃO: persistência de ocupação
        if d < MAX_RANGE_THRESHOLD:
            if len(rr) > 2:
                # Reduz impacto do livre se célula já tem forte evidência de ocupação
                l_free_eff = l_free * 0.3 if log_odds_map[posi_yGrid, posi_xGrid] > 2.0 else l_free
                log_odds_map[rr[1:-1], cc[1:-1]] += l_free_eff
            log_odds_map[posi_yGrid, posi_xGrid] += l_occ
        else:
            if len(rr) > 1:
                log_odds_map[rr[1:], cc[1:]] += l_free

        # 🔧 MODIFICAÇÃO: marca o tempo da última observação
